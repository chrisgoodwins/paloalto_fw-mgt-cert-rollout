...
#
# Requirements: requests, lxml, requests_ntlm, certsrv
#
# Python:       Version 3
#
###############################################################################
...
def menu_8():
    ...
    while run:
        ...
        if fwCSRs == []:
            ...
            break
        while True:
            caSvr_Address = input('Enter the IP/FQDN address of your CA server: ')
                ...
                ## Query the CA server for each of the request IDs to see if they have been approved... ##
                    ...
                    for reqID in reqData:
                        try:
                            ...
                            time.sleep(1)
                            print('\n\nThere was an unknown error:', sys.exc_info()[0])
                            raise
                            time.sleep(1)
                            run = False
                            reqIDcheck = False
        ...
        if reqIDcheck == True:
            ...
            for fw in fwCSRs:
                ...
                try:
                    ...
                    certFile.write(pemFile.decode("utf-8").replace('\r', ''))
                    certFile.close()
